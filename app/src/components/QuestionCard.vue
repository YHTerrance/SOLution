...
const copyQuestionUrl = (questionBase58PublicKey) => {
  try {
    copyUrl(`#/question/${questionBase58PublicKey}`);
    status.addSuccessStatus("Successfully copied question link!");
  } catch (error) {
    console.log(error);
    status.addErrorStatus("Failed to copy question link!");
  }
};
</script>
  ...
  <div>
    ...
    <div v-else class="px-8 py-4">
      ...
      <div class="flex justify-between mt-4 items-center">
        <icon-heart
          class="w-8 h-8 text-pink-500 cursor-pointer dark:hover:text-midnight-900"
          :isActive="liked"
          @click="liked = !liked"
        ></icon-heart>
        <icon-comment
